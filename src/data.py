...
def load_dataset(name, path='../data', train_size=None, test_size=None, batch_size=32):
    ...
    if name in ['MUTAG', 'ENZYMES', 'PROTEINS']:
        ...
        dataset = WebKB(root=os.path.join(path, 'WebKB'), name=name.lower(),transform= WebKBPreprocess(num_train_per_class=20, val_ratio=0.2, test_ratio=0.2)) #, geom_gcn_preprocess=True)
        data = dataset[0]
